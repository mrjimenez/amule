...
; --------------------------------------------------------------------
;
; Register every language NSIS ships a Contrib/Language file for, not
; just the ones aMule has a po/ catalog for. The MUI built-in pages
; (Welcome / License / Components / Directory / Install / Finish /
; standard buttons / error dialogs) come from NSIS's bundled .nlf
; files for free whenever a language is registered here, so the
; widest registration gives the widest coverage of the installer UI
...
; Languages aMule has a po/ catalog for can carry translated
; LangString entries (contributed via follow-up PRs); the rest fall
; back to the English value via NSIS's "first registered language
; wins for missing LangString entries" rule -- which is why English
; is registered first.
...
; Per-language LangString entries for every other registered language
; are auto-generated by packaging/windows/po-to-nsh.py from the
; matching po/<lang>.po files at installer build time. /NONFATAL lets
; the .nsi compile cleanly when the bridge hasn't been run (e.g. a
; developer invoking makensis directly without packaging/windows/
; build.sh); in that case the custom strings fall through to the
; English bodies above via NSIS's "first registered language wins for
; missing LangString entries" rule.
!include /NONFATAL "installer_strings_generated.nsh"
