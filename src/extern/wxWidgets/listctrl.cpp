...
public:
	...
	struct GeometryInfo
	{
		...
		{
			wxASSERT_MSG(m_rectAll.width <= w, _T("width can only be increased"));
...
public:
	...
	void DrawInReportMode(wxDC *dc, const wxRect &rect, const wxRect &rectHL, bool highlighted);
...
private:
	...
	bool SetAttributes(wxDC *dc, const wxListItemAttr *attr, bool highlight);
...
public:
	...
	bool HasHeader() const { return InReportView() && !HasFlag(wxLC_NO_HEADER); }
	...
	wxSize GetLineSize(size_t line) const { return GetLineRect(line).GetSize(); }
	...
	wxTextCtrl *GetEditControl() const { return m_textctrlWrapper ? m_textctrlWrapper->GetText() : NULL; }
	...
	wxGenericListCtrl *GetListCtrl() const { return wxStaticCast(GetParent(), wxGenericListCtrl); }
...
protected:
	...
	wxColour GetRuleColour() const { return wxSystemSettings::GetColour(wxSYS_COLOUR_3DLIGHT); }
...
void wxListItemData::SetItem(const wxListItem &info)
{
	...
	if (m_rect) {
		m_rect->x = m_rect->y = m_rect->height = 0;
...
void wxListLineData::CalculateSize(wxDC *dc, int spacing)
{
	...
	case wxLC_SMALL_ICON:
		...
		if (s.empty()) {
			lh = m_gi->m_rectLabel.width = m_gi->m_rectLabel.height = 0;
		} else // has label
		{
...
void wxListLineData::SetPosition(int x, int y, int spacing)
{
	...
	case wxLC_SMALL_ICON:
		...
		if (item->HasImage()) {
			m_gi->m_rectIcon.x =
				m_gi->m_rectAll.x + 4 + (m_gi->m_rectAll.width - m_gi->m_rectIcon.width) / 2;
		...
		if (item->HasText()) {
			if (m_gi->m_rectAll.width > spacing)
				...
				m_gi->m_rectLabel.x = m_gi->m_rectAll.x + (EXTRA_WIDTH / 2) + (spacing / 2) -
						      (m_gi->m_rectLabel.width / 2);
			m_gi->m_rectLabel.y =
				m_gi->m_rectAll.y + m_gi->m_rectAll.height + 2 - m_gi->m_rectLabel.height;
	...
	case wxLC_LIST:
		...
		if (item->HasImage()) {
			...
			m_gi->m_rectLabel.x =
				m_gi->m_rectAll.x + 4 + (EXTRA_WIDTH / 2) + m_gi->m_rectIcon.width;
		} else {
			m_gi->m_rectLabel.x = m_gi->m_rectAll.x + (EXTRA_WIDTH / 2);
		}
		break;
...
bool wxListLineData::SetAttributes(wxDC *dc, const wxListItemAttr *attr, bool highlighted)
{
	...
	if (highlighted || hasBgCol) {
		if (highlighted)
			...
			dc->SetBrush(*(wxTheBrushList->FindOrCreateBrush(
				attr->GetBackgroundColour(), wxBRUSHSTYLE_SOLID)));
...
#else
	{
		if (highlighted) {
			...
			wxRendererNative::Get().DrawItemSelectionRect(
				m_owner, *dc, m_gi->m_rectHighlight, flags);
...
void wxListLineData::DrawInReportMode(wxDC *dc, const wxRect &rect, const wxRect &rectHL, bool highlighted)
{
	...
	wxCoord x = rect.x + HEADER_OFFSET_X, yMid = rect.y + rect.height / 2;
#ifdef __WXGTK__
	...
	size_t col = 0;
	for (wxListItemDataList::compatibility_iterator node = m_items.GetFirst(); node;
...
void wxListLineData::DrawTextFormatted(
	wxDC *dc, const wxString &textOrig, int col, int x, int yMid, int width)
{
...
// other platforms, please go ahead and revise or remove the #ifdef.
		...
		wxRendererNative::Get().DrawHeaderButton(this, dc, wxRect(x, HEADER_OFFSET_Y, cw, ch), flags);
		...
		dc.DrawText(item.GetText(),
			xAligned + EXTRA_WIDTH,
			h / 2 - hLabel / 2); // HEADER_OFFSET_Y + EXTRA_HEIGHT );
	...
	if (x < w) {
		wxRendererNative::Get().DrawHeaderButton(this, dc, wxRect(x, HEADER_OFFSET_Y, w - x, h), 0);
	}
}
...
void wxListHeaderWindow::OnMouse(wxMouseEvent &event)
{
	...
	} else // not dragging
	{
		...
		int col, countCol = m_owner->GetColumnCount();
		...
		if (event.LeftDown() || event.RightUp()) {
			if (hit_border && event.LeftDown()) {
				if (SendListEvent(wxEVT_LIST_COL_BEGIN_DRAG, event.GetPosition())) {
					m_isDragging = true;
					m_currentX = x;
					CaptureMouse();
					DrawCurrent();
			...
			} else // click on a column
			{
				...
				SendListEvent(
					event.LeftDown() ? wxEVT_LIST_COL_CLICK : wxEVT_LIST_COL_RIGHT_CLICK,
					event.GetPosition());
			}
...
wxListTextCtrlWrapper::wxListTextCtrlWrapper(wxListMainWindow *owner, wxTextCtrl *text, size_t itemEdit)
...
, m_itemEdited(itemEdit)
{
	...
	m_owner->CalcScrolledPosition(rectLabel.x, rectLabel.y, &rectLabel.x, &rectLabel.y);
...
void wxListMainWindow::Init()
{
	...
	m_countVirt = 0;
	m_lineFrom = m_lineTo = (size_t)-1;
	m_linesPerPage = 0;

	m_headerWidth = m_lineHeight = 0;
	...
	m_current = m_lineLastClicked = m_lineSelectSingleOnUp = m_lineBeforeLastClicked = (size_t)-1;
...
: wxScrolledWindow(parent, id, pos, size, style | wxHSCROLL | wxVSCROLL, name)
{
	...
	m_highlightBrush = *(wxTheBrushList->FindOrCreateBrush(
		wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHT), wxBRUSHSTYLE_SOLID));
	...
	// looking distinctly muted vs. the focused selection.
	{
		...
		const wxColour _unfocused((_hl.Red() + _lb.Red()) / 2,
			(_hl.Green() + _lb.Green()) / 2,
			(_hl.Blue() + _lb.Blue()) / 2);
		m_highlightUnfocusedBrush =
			*(wxTheBrushList->FindOrCreateBrush(_unfocused, wxBRUSHSTYLE_SOLID));
	}
...
wxListLineData *wxListMainWindow::GetDummyLine() const
{
	...
	// otherwise
	if (!m_lines.IsEmpty() && m_lines[0].m_items.GetCount() != (size_t)GetColumnCount()) {
		self->m_lines.Clear();
	}
...
{
	return InReportView() ? GetLineRect(line) : GetLine(line)->m_gi->m_rectHighlight;
}
...
long wxListMainWindow::HitTestLine(size_t line, int x, int y) const
{
	...
	if (ld->HasText() || InReportView()) {
		wxRect rect = InReportView() ? GetLineRect(line) : GetLineLabelRect(line);
...
void wxListMainWindow::HighlightLines(size_t lineFrom, size_t lineTo, bool highlight)
{
	if (IsVirtual()) {
		...
		if (!m_selStore.SelectRange(lineFrom, lineTo, highlight, &linesChanged)) {
...
bool wxListMainWindow::HighlightLine(size_t line, bool highlight)
{
	...
	if (changed) {
		SendNotify(line, highlight ? wxEVT_LIST_ITEM_SELECTED : wxEVT_LIST_ITEM_DESELECTED);
	}
...
void wxListMainWindow::OnPaint(wxPaintEvent &WXUNUSED(event))
{
	...
	// Clearing must be done first thing because caching of the double-buffering causes artifacts
	// otherwise.
	dc.Clear();
	...
	if (InReportView()) {
		...
		if (IsVirtual()) {
			wxListEvent evCache(wxEVT_LIST_CACHE_HINT, GetParent()->GetId());
		...
		for (size_t line = visibleFrom; line <= visibleTo; line++) {
			...
			if (HasFlag(wxLC_OWNERDRAW)) {
				...
				GetLine(line)->DrawInReportMode(
					&dc, rectLine, GetLineHighlightRect(line), IsHighlighted(line));
			}
		}
		...
		if (HasFlag(wxLC_HRULES)) {
			...
			if (i == 0)
				i = 1; // Don't draw the first one
			for (; i <= visibleTo; i++) {
				dc.SetPen(pen);
				dc.SetBrush(*wxTRANSPARENT_BRUSH);
				dc.DrawLine(0 - dev_x, i * lineHeight, clientSize.x - dev_x, i * lineHeight);
			}
...
#else
			wxRendererNative::Get().DrawItemSelectionRect(
				this, dc, rect, wxCONTROL_CURRENT | wxCONTROL_FOCUSED);
...
void wxListMainWindow::SendNotify(size_t line, wxEventType command, const wxPoint &point)
{
...
wxTextCtrl *wxListMainWindow::EditLabel(long item, wxClassInfo *textControlClass)
{
	...
	wxASSERT_MSG(textControlClass->IsKindOf(CLASSINFO(wxTextCtrl)), "EditLabel() needs a text control");
...
bool wxListMainWindow::OnRenameAccept(size_t itemEdit, const wxString &value)
{
	...
	return !GetParent()->GetEventHandler()->ProcessEvent(le) || le.IsAllowed();
}
...
void wxListMainWindow::OnMouse(wxMouseEvent &event)
{
	...
	if (!(event.Dragging() || event.ButtonDown() || event.LeftUp() || event.ButtonDClick()))
		return;
	...
	size_t count = GetItemCount(), current;
	...
	if (event.Dragging()) {
		...
		int command = event.RightIsDown() ? wxEVT_LIST_BEGIN_RDRAG : wxEVT_LIST_BEGIN_DRAG;
	...
	if (event.LeftUp()) {
		...
		if (m_lastOnSame) {
			if ((current == m_current) && (hitResult == wxLIST_HITTEST_ONITEMLABEL) &&
	...
	if (event.RightDown()) {
		...
		wxContextMenuEvent evtCtx(
			wxEVT_CONTEXT_MENU, GetParent()->GetId(), ClientToScreen(event.GetPosition()));
		evtCtx.SetEventObject(GetParent());
		GetParent()->GetEventHandler()->ProcessEvent(evtCtx);
		...
		} else // multi sel & either ctrl or shift is down
		{
			if (cmdModifierDown) {
				...
				size_t lineFrom = oldCurrent, lineTo = current;
...
{
	wxCHECK_RET(newCurrent < (size_t)GetItemCount(), _T("invalid item index in OnArrowChar()"));
...
void wxListMainWindow::OnChar(wxKeyEvent &event)
{
	...
	case WXK_PAGEUP: {
		int steps = InReportView() ? pageSize - 1 : m_current % pageSize;
	...
	case WXK_PAGEDOWN: {
		int steps = InReportView() ? pageSize - 1 : pageSize - (m_current % pageSize) - 1;
...
{
	{
		wxCHECK_RET(col >= 0 && col < GetColumnCount(), _T("invalid column index"));
	}

	{
		wxCHECK_RET(InReportView(), _T("SetColumnWidth() can only be called in report mode."));
	}
...
void wxListMainWindow::SetItem(wxListItem &item)
{
	...
	wxCHECK_RET(id >= 0 && (size_t)id < GetItemCount(), _T("invalid item index in SetItem"));
...
void wxListMainWindow::SetItemState(long litem, long state, long stateMask)
{
	...
	wxCHECK_RET(
		litem >= 0 && (size_t)litem < GetItemCount(), _T("invalid list ctrl item index in SetItem"));
...
void wxListMainWindow::GetItem(wxListItem &item) const
{
	...
	if (item.m_mask & wxLIST_MASK_STATE)
		item.m_state = GetItemState(item.m_itemId, wxLIST_STATE_SELECTED | wxLIST_STATE_FOCUSED);
}
...
{
	wxASSERT_MSG(
		!HasFlag(wxLC_REPORT | wxLC_LIST), _T("wxListCtrl::GetViewRect() only works in icon mode"));
	...
	if (count) {
		for (int i = 0; i < count; i++) {
			...
			wxCoord x = r.GetRight(), y = r.GetBottom();
...
{
	wxCHECK_RET(index >= 0 && (size_t)index < GetItemCount(), _T("invalid index in GetItemRect"));

	// ensure that we're laid out, otherwise we could return nonsense
	if (m_dirty) {
		wxConstCast(this, wxListMainWindow)->RecalculatePositions(true /* no refresh */);
	}
			...
			for (int tries = 0; tries < 2; tries++) {
				...
				for (size_t i = 0; i < count; i++) {
					...
					if ((tries == 0) && (entireWidth + SCROLL_UNIT_X > clientWidth)) {
						clientHeight -= wxSystemSettings::GetMetric(wxSYS_HSCROLL_Y);
						m_linesPerPage = 0;
						break;
					}
...
long wxListMainWindow::GetNextItem(long item, int WXUNUSED(geometry), int state) const
{
	long ret = item, max = GetItemCount();
	wxCHECK_MSG((ret == -1) || (ret < max), -1, _T("invalid listctrl index in GetNextItem()"));
...
void wxListMainWindow::DeleteItem(long lindex)
{
	...
	wxCHECK_RET((lindex >= 0) && ((size_t)lindex < count), _T("invalid item index in DeleteItem"));
...
{
	wxCHECK_RET(index >= 0 && (size_t)index < GetItemCount(), _T("invalid index in EnsureVisible"));
...
long wxListMainWindow::FindItem(long start, wxUIntPtr data)
{
	...
	if (start <= 0) {
		...
		std::unordered_map<wxUIntPtr, size_t>::const_iterator it = m_dataIndex.find(data);
	...
	for (size_t i = (size_t)start; i < count; i++) {
		wxListItemDataList::compatibility_iterator node = m_lines[i].m_items.GetFirst();
...
void wxListMainWindow::InsertItem(wxListItem &item)
{
	...
	if (!m_dataIndexDirty && (item.m_mask & wxLIST_MASK_DATA) && id == m_lines.GetCount() - 1) {
...
{
	if (!m_linesPerPage) {
		wxConstCast(this, wxListMainWindow)->m_linesPerPage = GetClientSize().y / GetLineHeight();
	}
...
void wxListMainWindow::GetVisibleLinesRange(size_t *from, size_t *to)
{
	...
	wxASSERT_MSG(IsEmpty() || (m_lineFrom <= m_lineTo && m_lineTo < GetItemCount()),
...
{
	m_headerWin = new wxListHeaderWindow(this,
		wxID_ANY,
		m_mainWin,
		wxPoint(0, 0),
...
bool wxGenericListCtrl::Create(wxWindow *parent,
	...
	const wxString &name)
{
	m_imageListNormal = m_imageListSmall = m_imageListState = (wxImageList *)NULL;
	m_ownsImageListNormal = m_ownsImageListSmall = m_ownsImageListState = false;
...
{
	wxASSERT_MSG(!(style & wxLC_VIRTUAL), _T("wxLC_VIRTUAL can't be [un]set"));
...
bool wxGenericListCtrl::SetItemColumnImage(long item, long column, int image)
{
	wxListItem info;
	info.m_image = image;
	info.m_mask = wxLIST_MASK_IMAGE;
...
void wxGenericListCtrl::OnDrawItem(int WXUNUSED(item),
	wxDC *WXUNUSED(dc),
	const wxRect &WXUNUSED(rect),
	const wxRect &WXUNUSED(rectHL),
...
wxTextCtrl *wxGenericListCtrl::EditLabel(long item, wxClassInfo *textControlClass)
{
...
long wxGenericListCtrl::FindItem(long WXUNUSED(start), const wxPoint &pt, int WXUNUSED(direction))
{
...
long wxGenericListCtrl::InsertColumn(long col, const wxString &heading, int format, int width)
{
	wxListItem item;
...
void wxGenericListCtrl::ResizeReportView(bool showHeader)
{
	...
	if (showHeader) {
		...
		if (ch > m_headerHeight)
			...
			m_mainWin->SetSize(0, m_headerHeight + 1, cw, 0);
...
// static
wxVisualAttributes wxGenericListCtrl::GetClassDefaultAttributes(wxWindowVariant variant)
{
#if _USE_VISATTR
...
int wxGenericListCtrl::OnGetItemImage(long WXUNUSED(item)) const
{
	wxCHECK_MSG(!GetImageList(wxIMAGE_LIST_SMALL),
		-1,
		"List control has an image list, OnGetItemImage or OnGetItemColumnImage should be "
		"overridden.");
	return -1;
}
...
wxListItemAttr *wxGenericListCtrl::OnGetItemAttr(long WXUNUSED_UNLESS_DEBUG(item)) const
{
	wxASSERT_MSG(item >= 0 && item < GetItemCount(), _T("invalid item index in OnGetItemAttr()"));
